...
-module(gms_networkbytesReceived_monitor,[BASE]).
...
getcounterids()->
	[{"BytesReceivedPersec","PERF_COUNTER_COUNTER"}
	].
%%
%% 
objects()->
...
getCounters()->
	{ok,{_,NetworkInterface}}=THIS:get_property(networkInterface),
	{ok,[#perf_counter{object="Network Interface",counterName="Bytes Received/sec",instance=iconv:convert("utf-8", "gbk",NetworkInterface)}]}.
update()->
...
make_state_string([{K,V}|T],Measure,MeasureBase,Scale,DefaLastMeasure,Tab,Cur,Max)->
	...
	if
		Val >= 0 ->
            ...
            THIS:set_attribute(bytesReceivedPersec,Result), 
			THIS:set_attribute(status,"ok"),
			KK="Received Bytes/sec",
			if
				is_number(FResult) ->
...
buildInstances([H|E],R)->
	...
	Temp=case Sinstance of
			 "_Total" ->
				 R;
			 _ ->
				 R++[{iconv:convert("gbk", "utf-8",Sinstance),iconv:convert("gbk", "utf-8",Sinstance)}]
		 end,
    buildInstances(E,Temp).
getInstances(Host,User,Passwd)->
	 Ssql=["select name from Win32_PerfRawData_Tcpip_NetworkInterface"],
...
get_template_property()->
	...
	[
	#property{name=networkInterface,title="Network Interface",type=scalar,editable=true,order=1},
	#property{name=bytesReceivedPersec,title="Received Bytes/sec",type=numeric,configurable=false,state=true},
