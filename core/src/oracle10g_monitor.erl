...
%% Created: 2010-7-22
%% Description: TODO: Add description to oracle10g_monitor
-module(oracle10g_monitor,[BASE]).
-compile(export_all).
...
getBrowseData(Params) ->
	...
	MetricType=proplists:get_value(pMetricType,Params),
    if Secure ->
        S8 = "https" ++ "://" ++ Hostname ++ ":"  ++ Port ++ "/"++MetricType++ "/Spy?format=xml";
    true ->    
        S8 = "http" ++ "://" ++ Hostname ++ ":"  ++ Port ++  "/"++MetricType++ "/Spy?format=xml" 
    end,
...
%% 		case Version of
%% 			"7.x" ->
%% 				Response = rpc(?REG_NAME, Java_Node, {"com.dragonflow.erlangecc.monitor.Oracle10gMonitor", "getBrowseData", [{xml,iconv:convert("utf-8", "gbk", Data)}]}, ?TIMEOUT);
%% 			_ ->
				Response = rpc(?REG_NAME, Java_Node, {"com.dragonflow.erlangecc.monitor.Oracle10gMonitor", "getBrowseData", [{xml, Data}]}, ?TIMEOUT)
%% 		end
...
%% 	{"4.x","4.x"},{"5.x","5.x"},
	case Prop of
		pMetricType ->
			[{"App Server","dmsoc4j"},{"Web Server","dms0"}];
		_ ->
...
update() ->
    ...
    {ok,{_,Browse}} = THIS:get_property(browse),  
	{ok,{_,MetricType}} = THIS:get_property(pMetricType),  
    ...
    true ->    
        if Flag ->
             S8 = "https" ++ "://" ++ Hostname ++ ":"  ++ integer_to_list(getPort()) ++ "/"++MetricType++ "/Spy?format=xml";
        true ->    
            S8 = "http" ++ "://" ++ Hostname ++ ":"  ++ integer_to_list(getPort()) ++  "/"++MetricType++ "/Spy?format=xml"
        end,
        {Status,Data} = http_receive:receive_asynchronous_data(S8,Proxy,getPort(),ProxyUser,ProxyPassword,Time*1000),
%% 		io:format("Status:~p ~n", [Time*1000]),
        case Status of
        error ->
...
%% 		io:format("data:~p ~n", [Data]),
		    Response = rpc(?REG_NAME, Java_Node, {"com.dragonflow.erlangecc.monitor.Oracle10gMonitor", "update", [{xml, Data},{counters,Browse}]}, ?TIMEOUT),
...
setCounterValue({Key,Value})->
	THIS:set_attribute(convert1(Key),convert1(Value)).
convert([H|R], Acc)->
	case H > 255 of
		true->
			<<H1:8, H2:8>> = <<H:16>>,
...
convert([], Acc)->
%% 	lists:reverse(Acc).
 	iconv:convert("gbk", "utf-8", lists:reverse(Acc)).
...
get_template_property() ->
    ...
    [   
        ...
        #property{name=pPort,title="Port",type=numeric,editable=true,order=2,default=7201,description="port number on server - default is 7201 and is configured in dms.conf"},
        #property{name=pMetricType,title="Metric Type",type=scalar,description="Type of metrics to monitor. Choices are App Server (OC4J) and Web Server(dms).",order=4},
        ...
        #property{name=pUser,title="Authorize User",type=text,order=9,editable=true,description="the username for the Authorize"},
        #property{name=pPassword,title="Authorize Password",type=password,order=10,editable=true,description="the password for the Authorize"},
