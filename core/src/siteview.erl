...
init_application(AppName) ->     
     case init_profile() of
           {ok,Profileid} ->
                case rpc_proxy:call(?DBName,app,get,[[{application,AppName}]]) of 
                {ok,{_,_,_,[]}} -> 
                 Application = #application{ 
                  ...
                  case rpc_proxy:call(?DBName,app,create,[[{version,2}],Application]) of 
                        {ok,App} -> {ok,App};
...
init_profile(UserName) ->      
     case rpc_proxy:call(?DBName,profile,get,[[{application,"oldhand"},{profile,"email="++UserName}]]) of 
            {ok,{_,_,_,[]}} -> 
             Profile = #profile{ 
					 id=null, 
                     title = <<"oldhand">>, 
              ...
              case rpc_proxy:call(?DBName,profile,create,[[{version,2}],Profile]) of 
