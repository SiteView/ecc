-module(newrock_callsDropped_monitor,[BASE]).
...
update() ->
    ...
    if length(ParametersT) == 0 ->
        Parameters = [
                    "Device.Services.VoiceService.1.VoiceProfile.1.Line.1.Stats.CallsDropped"
                    ];
    true ->                
...
set_parameter(Number,Browse,ValueList) ->
    case [ element(4,X)||X<-ValueList,string:str(element(3,X),"CallsDropped")>0] of
    [] ->
        %%String1 = "Dropped Calls = n/a <br>", 
        %%THIS:set_attribute(callsDropped,"n/a"); 
        String1=[];
    CallsDroppedNumList ->
        case Number of
        0 ->
            String1 = "Dropped Calls = 0 <br>", 
            THIS:set_attribute(callsDropped,0);
        _ -> 
            String1 = "Dropped Calls = " ++ integer_to_list(add(CallsDroppedNumList)) ++ "<br>", 
            THIS:set_attribute(callsDropped,add(CallsDroppedNumList))
        end            
    end,
...
getStateProperties(This,Params) ->
   %%[
   %%     #property{name=callsDropped,title="Dropped Calls",type=numeric,state=true,configurable=false}
    %%].
   [].
...
getBrowseData(Params) -> 
    ...
    _ ->         
        ...
        _ ->
            [{"Line1CallsDropped","Line1CallsDropped"}]
        end
    end.  
...
%% 安装
make_cur_line(Isdn) ->
    [{"ISDN"++integer_to_list(Isdn)++"CallsDropped","CallsDropped"}].
...
make_isdn_t(0,R) -> R;
make_isdn_t(I,Res) ->
    make_isdn_t(I-1,Res ++ [{"ISDN"++integer_to_list(I)++"CallsDropped","ISDN"++integer_to_list(I)++"CallsDropped"}]). 
