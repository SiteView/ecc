...
-module(gms_disktotalreads_monitor,[BASE]).
...
getCounters()->
	{ok,[#perf_counter{object="LogicalDisk",counterName="Disk Read Bytes/sec",instance="_Total"}]}.
update()->
...
make_state_string([{K,V}|T],Measure,MeasureBase,Scale,DefaLastMeasure,Tab,Cur,Max)->
	...
	if
		Val >= 0 ->
            ...
            THIS:set_attribute(totalDiskReadBytesPersec,Result), 
			THIS:set_attribute(status,"ok"),
			KK="Total Disk Read Bytes/sec",
			if
				is_number(FResult) ->
...
get_template_property()->
	...
	[
	#property{name=totalDiskReadBytesPersec,type=numeric,title="Total Disk Read Bytes/sec",configurable=false,state=true},
