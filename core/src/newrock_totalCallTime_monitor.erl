-module(newrock_totalCallTime_monitor,[BASE]).
...
update() ->
    ...
    io:format("Browse !!!!!!!!!!!!!!!!~n~p~n", [Browse]),
    %%
	ParametersT = get_parameters(Browse,[],CpeDevice),
    if length(ParametersT) == 0 ->
        Parameters = [
                    "Device.Services.VoiceService.1.VoiceProfile.1.Line.1.Stats.TotalCallTime"
                    ];
    true ->                    
...
set_parameter(Number,Browse,ValueList) ->
    case [ element(4,X)||X<-ValueList,string:str(element(3,X),"TotalCallTime")>0] of
    [] ->
        %%String1 = "Call Time Total = n/a <br>", 
        %%THIS:set_attribute(totalCallTime,"n/a"); 
        String1=[];
    TotalCallTimeNumList ->
        case Number of
        0 ->
            String1 = "Call Time Total = 0" ++ "<br>", 
            THIS:set_attribute(totalCallTime,0);
        _ -> 
            String1 = "Call Time Total = " ++ integer_to_list(add(TotalCallTimeNumList)) ++ "<br>", 
            THIS:set_attribute(totalCallTime,add(TotalCallTimeNumList))
        end            
    end,
    set_parameter_t(Browse,length(Browse),ValueList,String1).
...
getStateProperties(This,Params) ->
   %%[
   %%     #property{name=totalCallTime,title="Call Time Total",type=numeric,state=true,configurable=false}
    %%].
   [].
...
getBrowseData(Params) -> 
    ...
    _ ->         
        ...
        _ ->
            [{"Line1TotalCallTime","Line1TotalCallTime"}]
        end
    end.  
...
%% 安装
make_cur_line(Isdn) ->
    [{"ISDN"++integer_to_list(Isdn)++"TotalCallTime","TotalCallTime"}].
...
make_isdn_t(0,R) -> R;
make_isdn_t(I,Res) ->
    make_isdn_t(I-1,Res ++ [{"ISDN"++integer_to_list(I)++"TotalCallTime","ISDN"++integer_to_list(I)++"TotalCallTime"}]). 
