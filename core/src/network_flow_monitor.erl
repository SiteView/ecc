-module(network_flow_monitor,[BASE]).
-compile(export_all).
...
update() ->
    Info = case THIS:get_property(browse) of
	       {ok,{_,Counters}} ->		
		       ...
		       UniqueFormulas = nnm_monitor_util:unique(nnm_monitor_util:get_selected_formulas(Formulas)),
		       SelectedFormulaTypes = nnm_monitor_util:unique([nnm_monitor_util:get_formula_type(Item) || Item <- UniqueFormulas]),
		       Data = network_flow:get_formulas_data(SelectedFormulaTypes,FinalCounters,flow),
		       Infos = nnm_monitor_util:get_data_desc(Data),
		       %io:format("Data:~p,Infos:~p~n",[Data,Infos]),
		       [THIS:set_attribute(Formula,Value) || {Formula,Value,_Unit} <- network_flow:get_formula_value(Data,UniqueFormulas)],	   
