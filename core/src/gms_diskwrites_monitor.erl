...
-module(gms_diskwrites_monitor,[BASE]).
...
getCounters()->
	...
	{ok,[#perf_counter{object="LogicalDisk",counterName="Disk Write Bytes/sec",instance=Disk}]}.
update()->
...
make_state_string([{K,V}|T],Measure,MeasureBase,Scale,DefaLastMeasure,Tab,Cur,Max)->
	...
	if
		Val >= 0 ->
            ...
            THIS:set_attribute(diskWriteBytesPersec,Result), 
			THIS:set_attribute(status,"ok"),
			KK="Disk Write Bytes/sec",
			if
				is_number(FResult) ->
...
get_template_property()->
	...
	#property{name=diskWriteBytesPersec,type=numeric,title="Disk Write Bytes/sec",configurable=false,state=true},
