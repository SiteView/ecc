...
%% @doc NTcounter Base.
%% 
%% Description: Provide basic function for ASP,IIS,SQL,ADserver monitor
%% using perfix.exe
-module(ntcounter_base,[BASE]).
...
getPerformanceData(Machine,PageCounters)->
    %%io:format("get from page counters is:~p~n",[buildcounters(Counters)]),
   ...
   LastMeasurement =  case THIS:get_attribute(list_to_atom("lastMeasurement"++integer_to_list(N))) of
	...
	case is_number(PerfResult2#perf_result.value) of
		false->
			...
			S4 = PerfCounter#perf_counter.counterName ++ " " ++ "n/a" ++ "<br>";
		_->
			...
			S4 = PerfCounter#perf_counter.counterName++" = "++NewPP++"<br>"
	end,
