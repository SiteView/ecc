...
-module(browsableNTCounter_monitor,[BASE]).
-compile(export_all).
...
%getPerfData(Host,Counters,Monitor,Map),Run only once
...
update_for(Pnci,Pncv,Pncn,Num,Max,Int) ->
    {ok,{_,S2}} = THIS:get_property(Pnci ++ integer_to_list(Num+1)), %S2 format "33 LogicalDiskx_TotalxFreexMegabytes"
...
getBrowseData(Params) ->
	case proplists:get_value(pFile,Params) of
		undefined->
			[];
		File->
...
getScalarValues(Prop,Params)->
	case Prop of
		pFile ->
            ...
            Path =  "templates.perfmon/browsable",
...
makeScalar_t(FL,Num,P,Sep,N,Li) ->
    [A|B] = FL,
    Bool1 =  (string:str(A,".xml") > 0),
    Bool2 = (string:str(A,".XML") > 0 ),
    if Bool1 or Bool2 ->
        makeScalar_t(B,Num-1,P,Sep,N+1,lists:append(Li,[{A,P++"/"++A}]));
    true ->
        makeScalar_t(B,Num-1,P,Sep,N+1,Li)
    end.
...
findCounters(Document,Hostname) ->
...
buildBrowseString_for_t(Llist,Map,Num,El) ->
    ...
    if length(Map1) > 0 -> 
        L = buildBrowseString_for_if_while_while(Map1,Hashset1,Hashset);
    true ->
        L = []
    end,
...
buildBrowseString_for_if_while_while(Map1,Hashset1,Hashset) -> %
    buildBrowseString_for_if_while_while_t(Map1,Hashset1,Hashset,length(Map1),[]).
buildBrowseString_for_if_while_while_t(_Map1,_Hashs1,_Hashs,0,E) -> E;  
buildBrowseString_for_if_while_while_t(Map,Hashs1,Hashs,Num,El) ->
    [A|B] = Map, 
    {perf_counter,Obj,Counter,Ins,_,_} = A,
    if  Hashs /= null ->
        case lists:member(Ins,Hashs) of 
        true ->    
            case lists:member(Counter,Hashs1) of
            true ->
                String = createBrowseId(A),
                THIS:set_property(String,A),
                buildBrowseString_for_if_while_while_t(B,Hashs1,Hashs,Num-1,lists:append(El,[{String,Obj ++ "/" ++ Ins ++ "/" ++Counter}]));
            _ ->
                buildBrowseString_for_if_while_while_t(B,Hashs1,Hashs,Num-1,El)
            end;
         _ ->
            buildBrowseString_for_if_while_while_t(B,Hashs1,Hashs,Num-1,El)
        end;
...
buildcounters([])->"";
buildcounters([F|R])->
	iconv:convert("gbk","utf-8",F#perf_counter.object) ++ " -- " ++ iconv:convert("gbk","utf-8",F#perf_counter.counterName) ++ " -- " ++ iconv:convert("gbk","utf-8",F#perf_counter.instance) ++ "," ++ buildcounters(R).
...
getPerfCounters(Machine,Object,S)->
	...
	Array1 = perfCounters(string:tokens(iconv:convert("gbk","utf-8",Rets),"\r\n"),Object),
