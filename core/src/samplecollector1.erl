...
%% 	ets:insert(Obj:get_tid(), {bucketsTemp, []}),	
	...
	ets:insert(Obj:get_tid(), {compress, 1}),
	{?MODULE,Obj}.
...
%% 	L4 = L3/ Precision,
	io:format("samplecollectordata:createBuckets1: ~p ~n", [THIS:get_tid()]),
	Buckets = constructBuckets(0, [], round((Length/(Length/200+1))), StartTime),
...
%% 	io:format("~p ~n", [lists:nth(10, T1)]),	
	...
	ets:insert(THIS:get_tid(), {compress, Length/200+1}).
%% 	io:format("samplecollectordata:createBuckets2: ~p ~n", [THIS:get_tid()]).
