%% @author bin.liu
%% @copyright 2011 dragonflow, Inc.
%% @version 1.0
%% @doc citrix Monitor.
%% 
%% Description: citrix Monitor
%% Versions supported: citrix app 6.5, 7.1, 2000, 2005, and 2008
%% Platform: Windows
-module(citrix_monitor,[BASE]).
...
getBrowseData(Params)->
	case file:consult("templates.applications/counters.citrix") of
		{ok,File}->
		    S=File;
...
buildTree([],Tree)->
	Tree;
buildTree([H|E],Tree)->
	{Obj,Instance,Counters}=H,
	Treea=case Instance of 
			  [] ->
				  [{Obj,Obj}]++[{Obj++":"++Counter,Obj++"/"++Counter}||Counter<-Counters];
			  _ ->
				  [{Obj,Obj}]++[{Obj++Instance,Obj++"/"++Instance}]++[{Obj++":"++Counter++":"++Instance,Obj++"/"++Instance++"/"++Counter}||Counter<-Counters]
			  end,
	T=Treea++Tree,
	buildTree(E,T).
...
%% object,counterName,instance,objectID,counterID
buildPerCounters([],Acc)->
	lists:reverse(Acc);
buildPerCounters([H|E],Acc)->
	{K,V}=H,
	Ls=string:tokens(V, "/"),
	case length(Ls) of
		3 ->
			Obj=lists:nth(1, Ls),
			Instance=lists:nth(2,Ls),
			Counter=lists:nth(3,Ls);
		    
		_ ->
			Obj=lists:nth(1, Ls),
			Counter=lists:nth(2,Ls),
	        Instance=""
	end,
	PerCounter=#perf_counter{object=Obj,
							 counterName=util:replace(Counter," Per ","/"),
							 instance=Instance
							 },
	buildPerCounters(E,[PerCounter|Acc]).
...
getPerformanceData(Machine,PageCounters)->
    ...
    Counters = buildPerCounters(PageCounters,[]),
	...
	StateString = make_state_string(Array,Flag,NewPerfResult,1,Counters),
...
make_state_string([{_,V}|T],Flag,PerfResult,N,Counters)->
	PerfCounter =lists:nth(N,Counters),
...
bc(F = #perf_counter{}) ->
	case F#perf_counter.instance of
		[] ->
			F#perf_counter.object ++ ":" ++ F#perf_counter.counterName;
		_ ->
			F#perf_counter.object ++ ":"++F#perf_counter.instance++":"++ F#perf_counter.counterName
	end;
bc(_) ->
    "".
...
get_template_property()->
	...
	[
	#property{name=browse,title="Counters", description="Current selection of counters.",type=browsable,editable=true,order=1},
