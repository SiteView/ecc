...
-module(gms_networkbytessent_monitor,[BASE]).
...
getcounterids()->
	[{"BytesSentPersec","PERF_COUNTER_COUNTER"}
	].
%%
%% 
objects()->
...
getCounters()->
	...
	{ok,[#perf_counter{object="Network Interface",counterName="Bytes Sent/sec",instance=iconv:convert("utf-8", "gbk",NetworkInterface)}]}.
update()->
...
make_state_string([{K,V}|T],Measure,MeasureBase,Scale,DefaLastMeasure,Tab,Cur,Max)->
	...
	if
		Val >= 0 ->
            ...
            THIS:set_attribute(bytesSentPersec,Result), 
...
get_template_property()->
	...
	#property{name=bytesSentPersec,title="Sent Bytes/sec",type=numeric,configurable=false,state=true},
