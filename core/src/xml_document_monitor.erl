...
-module(xml_document_monitor,[BASE]).
    ...
    {ok,_} ->    
        if length(Xsl) > 0 ->
            ...
            {ok,Binary} ->
                ...
                StatusString = update_util(xml_document_parser:parser({content,Response}),Browse),
                THIS:set_attribute(?STATE_STRING,StatusString);                         
            _ ->
                case http_receive:receive_asynchronous_data(Url,"","","","",30000) of
                {ok,Res} ->
                    StatusString = update_util(xml_document_parser:parser({content,Res}),Browse), 
                    THIS:set_attribute(?STATE_STRING,StatusString);                    
                _ ->
...
update_util_t(Li,Brow,Num,En) ->
    ...
    {value,{_,V}} ->
        ...
        true ->     
			Result = try list_to_float(V) of
						Float -> Float
					catch 
						_:_ -> list_to_integer(V)
					end,
            THIS:set_attribute(N,Result)
        end;
    _ ->
        V = "n/a",
        THIS:set_attribute(N,"n/a")
...
getBrowseData(Params) ->
    ...
    if length(Xsl) > 0 ->
        ...
        {ok,Binary} ->               
            ...
            {ok,_} ->
                ...
                make_tree(getBrowseData_util(xml_document_parser:parser({content,Response}))); 
            _ ->
                []  
            end; 
        _->
    ...
    true ->
        case http_receive:receive_asynchronous_data(Url,"","","","",30000) of
        {ok,Response} ->
			%~ io:format("parser:~p~n",[xml_document_parser:parser({content,Response})]),
            Tree = make_tree(getBrowseData_util(xml_document_parser:parser({content,Response}))),
