...
-module(gms_disktotalwrites_monitor,[BASE]).
...
getCounters()->
	{ok,[#perf_counter{object="LogicalDisk",counterName="Disk Write Bytes/sec",instance="_Total"}]}.
update()->
	...
	if
		Val >= 0 ->
			...
			case FResult of
 			...
 			io:format("keykeykkkkk:~p~n", [K]),
            THIS:set_attribute(totalDiskWriteBytesPersec,Result), 
			THIS:set_attribute(status,"ok"),
			KK="Total Disk Write Bytes/sec",
			if
				is_number(FResult) ->
...
get_template_property()->
	...
	[
	#property{name=totalDiskWriteBytesPersec,type=numeric,title="Total Disk Write Bytes/sec",configurable=false,state=true},
