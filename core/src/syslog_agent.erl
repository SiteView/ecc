...
%% Exported
-export([start/0,stop/0,version/0,send/3,send/4,send/5,send/6]).
...
send(Who,Level,Msg) when atom(Who),integer(Level),list(Msg) ->
    ?SERVER_NAME ! {send,{Who,Level,Msg}, self()}.
    
send(TagHost, TagPort, Who,Level,Msg) when atom(Who),integer(Level),list(Msg) ->
    ?SERVER_NAME ! {send, TagHost, TagPort, {Who,Level,Msg}, self()}.

send(Facility,Who,Level,Msg) when integer(Facility),atom(Who),integer(Level),list(Msg) ->
    ?SERVER_NAME ! {send,{Facility,Who,Level,Msg}, self()}.

send(TagHost, TagPort, Facility,Who,Level,Msg) when integer(Facility),atom(Who),integer(Level),list(Msg) ->
    ?SERVER_NAME ! {send,TagHost, TagPort,{Facility,Who,Level,Msg}, self()},
    receive
        ok ->
            {ok, success};
        {ok, Result} ->
...
loop(S,Host,Port) ->
    receive
    {send, TagHost, TagPort, What, From} ->
        do_send(S,TagHost,TagPort,What, From),
	    loop(S,Host,Port);
	{send,What, From} ->
	    do_send(S,Host,Port,What, From),
	    loop(S,Host,Port);
	{From,stop} ->
	    From ! stopped;
	_ ->
...
do_send(S,Host,Port,{Who,Level,Msg}, From) ->
    Packet = "<" ++ i2l(Level) ++ "> " ++ a2l(Who) ++ ": " ++ Msg ++ "\n",
    gen_udp:send(S,Host,Port,Packet);
do_send(S,Host,Port,{Facil,Who,Level,Msg}, From) ->
    Packet = "<" ++ i2l(Facil bor Level) ++ "> " ++ a2l(Who) ++ ": " ++ Msg ++ "\n",
    Result = (catch gen_udp:send(S,Host,Port,Packet)),
    From ! Result,
