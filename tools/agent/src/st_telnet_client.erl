...
loop(State, Sock, Acc) ->
	...
	{tcp,_,Msg0} ->
	    dbg("tcp msg: ~p~n",[{Msg0,Acc}]),
	    Msg = check_msg(Sock,Msg0,Acc,[]),
	...
	{send_data,Data} ->
	    ...
	    loop(State, Sock, Acc);    
	{get_data,Pid} ->
	    dbg("get_data: ~p~n",[Acc]),
	    NewState = 
		case Acc of
		    [] ->
			dbg("get_data nodata\n",[]),			
			erlang:send_after(2800,self(),{get_data_delayed,Pid}),
...
check_msg(Sock, [?IAC,?IAC | T],Msg, Acc) ->
    check_msg(Sock, T,Msg, [?IAC|Acc]);

%% respond to a command
check_msg(Sock, [?IAC | Cs],Msg, Acc) ->
    case get_cmd(Cs) of
	{Cmd,Cs1} ->
	    dbg("Got ~n", []), 
	    cmd_dbg("recv:",Cmd),	   
	    respond_cmd(Cmd,Cs1, Sock),
	    check_msg(Sock, Cs1,Msg, Acc); 
	error ->
	    Acc
    end;

%% buffer a data value
check_msg(Sock, [H|T],Msg, Acc) ->
    check_msg(Sock, T,Msg, [H|Acc]);
    

check_msg(_Sock, [],_, Acc) ->
    Acc.
...
respond_cmd([?DO,?WINDOW_SIZE],_Rest,Sock) -> 
   R = [?IAC,?WILL,?WINDOW_SIZE],
   cmd_dbg("send:",R),
   gen_tcp:send(Sock, R),   
   send([?IAC,?SB,?WINDOW_SIZE,0,200,0,200,?IAC,?SE], Sock);

respond_cmd([?WILL,?ECHO],_Rest,Sock) ->   
    R = [?IAC,?DO,?ECHO],
    cmd_dbg("send:",R),
    gen_tcp:send(Sock, R);

respond_cmd([?DO,?ECHO],_Rest,Sock) ->
...
respond_cmd([?WILL,?SUPPRESS_GO_AHEAD],_Rest,_Sock) ->
    dbg("Server will suppress-go-ahead\n", []);

respond_cmd([?WONT,?SUPPRESS_GO_AHEAD],_Rest,_Sock) ->
    dbg("Warning! Server won't suppress-go-ahead\n", []);

respond_cmd([?DONT | _Opt],_Rest,_Sock) ->		% server ack?
    ok;						
respond_cmd([?WONT | _Opt],_Rest, _Sock) ->		% server ack?
    ok;						

%% Negative responses (WON'T and DON'T). These are default!


respond_cmd([?WILL,Opt],_Rest, Sock) ->
    R = [?IAC,?DONT,Opt],    
    cmd_dbg("send:",R),
    gen_tcp:send(Sock, R);

respond_cmd([?DO | Opt],_Rest, Sock) ->
    R = [?IAC,?WONT | Opt],
    cmd_dbg("send:",R),
    gen_tcp:send(Sock, R);
...
respond_cmd(?NOP,_Rest, _Sock) ->
    ok;

%% Unexpected messages.

respond_cmd([Cmd | Opt],_Rest, _Sock) when Cmd >= 240, Cmd =< 255 ->
    dbg("Received cmd: ~w. Ignored!\n", [[Cmd | Opt]]);

respond_cmd([Cmd | Opt],_Rest, _Sock)  ->
