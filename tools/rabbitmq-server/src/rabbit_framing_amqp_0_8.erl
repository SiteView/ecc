...
-type(amqp_method_field_name() ::
       ( delivery_tag | integer_2 | heartbeat | active
       | string_result | immediate | multiple | delivery_tag
       | no_ack | nowait | consumer_tag | version_major
       | arguments | exclusive | content_size | challenge
       | mechanism | ticket | prefetch_size | nowait
       | ticket | requeue | no_ack | insist
       | auto_delete | result | routing_key | reply_text
       | consumer_tag | exchange | active | known_hosts
       | integer_result | global | message_count | consumer_tag
       | no_local | operation | realm | queue
       | routing_key | message_count | identifier | response
       | internal | message_count | consumer_tag | identifier
       | routing_key | nowait | frame_max | nowait
       | immediate | exchange | capabilities | exclusive
       | exchange | operation | nowait | reply_code
       | prefetch_count | details | consumer_tag | delivery_tag
       | consumer_tag | ticket | ticket | exchange
       | routing_key | client_properties | server_properties | global
       | channel_max | if_unused | mandatory | ticket
       | virtual_host | durable | nowait | reply_text
       | reply_code | consumer_count | identifier | queue
       | reply_code | arguments | mandatory | class_id
       | integer_3 | result | reply_text | content_checksum
       | queue | consume_rate | if_empty | prefetch_count
       | consumer_tag | queue | exchange | routing_key
       | durable | delivery_tag | ticket | exchange
       | routing_key | requeue | string_1 | passive
       | exclusive | reply_code | meta_data | nowait
       | exchange | ticket | routing_key | class_id
       | ticket | routing_key | ticket | prefetch_size
       | table | read | ticket | redelivered
       | consumer_tag | consumer_tag | exchange | out_of_band
       | response | ticket | exchange | heartbeat
       | queue | no_ack | host | queue
       | consumer_tag | delivery_tag | integer_1 | routing_key
       | version_minor | passive | queue | exchange
       | prefetch_size | method_id | queue | routing_key
       | consumer_tag | requeue | write | delivery_tag
       | dtx_identifier | locales | type | staged_size
       | if_unused | queue | global | consumer_tag
       | frame_max | ticket | no_local | active
       | redelivered | consumer_tag | reply_text | ticket
       | ticket | mechanisms | auto_delete | reply_text
       | integer_4 | exchange | nowait | exchange
       | cluster_id | exchange | nowait | prefetch_count
       | requeue | channel_max | consumer_tag | immediate
       | string_2 | known_hosts | arguments | delivery_tag
       | exchange | method_id | queue | multiple
       | reply_code | exclusive | routing_key | reply_code
       | reply_text | redelivered | exclusive | passive
       | message_count | string_op | nowait | integer_op
       | ticket | nowait | no_local | locale
