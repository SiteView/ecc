...
-type(amqp_method_field_name() ::
       ( ticket | version_minor | redelivered | message_count
       | if_unused | channel_max | known_hosts | delivery_tag
       | auto_delete | consumer_tag | requeue | arguments
       | consumer_tag | ticket | realm | prefetch_count
       | arguments | ticket | consumer_tag | redelivered
       | heartbeat | queue | server_properties | internal
       | nowait | arguments | arguments | ticket
       | prefetch_size | arguments | exclusive | reply_text
       | queue | nowait | active | requeue
       | consumer_tag | delivery_tag | nowait | routing_key
       | message_count | ticket | exchange | queue
       | routing_key | nowait | insist | nowait
       | active | ticket | routing_key | auto_delete
       | multiple | exclusive | immediate | response
       | arguments | version_major | durable | mechanism
       | exchange | frame_max | routing_key | consumer_tag
       | routing_key | read | method_id | exclusive
       | active | delivery_tag | consumer_count | mandatory
       | if_unused | locale | exchange | if_empty
       | ticket | queue | source | virtual_host
       | source | ticket | requeue | no_local
       | durable | nowait | delivery_tag | message_count
       | method_id | nowait | write | class_id
       | heartbeat | nowait | challenge | no_ack
       | client_properties | ticket | destination | destination
       | reply_text | reply_code | passive | channel_id
       | queue | class_id | exchange | capabilities
       | exchange | queue | queue | routing_key
       | response | delivery_tag | ticket | ticket
       | nowait | reply_code | reply_code | exchange
       | queue | no_ack | reply_text | exchange
       | passive | message_count | locales | ticket
       | exchange | channel_max | routing_key | mechanisms
       | arguments | nowait | frame_max | cluster_id
       | type | global | requeue | passive
