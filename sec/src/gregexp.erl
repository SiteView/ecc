...
reg4([C|_]) -> throw({error,{illegal,[C]}});
reg4([]) -> {epsilon,[]}.
...
char_class([C1|S0], Cc) when C1 /= $] ->
	...
	{Cf,[$-,C2|S1]} when C2 /= $] ->
		...
		{Cl,_} -> throw({error,{char_class,[Cf,$-,Cl]}})
	    end;
...
in_char_class(C, [{C1,C2}|_]) when C >= C1, C =< C2 -> true;
in_char_class(C, [C|_]) -> true;
in_char_class(C, [_|Cc]) -> in_char_class(C, Cc);
in_char_class(_, []) -> false.
...
re_apply_or({match,P1,S1,G1},{match,P2,_,_}) when P1>=P2 -> {match,P1,S1,G1};
re_apply_or({match,_,_,_}, {match,P2,S2,G2}) -> {match,P2,S2,G2};
...
first_match(RE, S, St) when S /= [] ->
	...
	{match,P,_,_Groups} -> {St,P-St};
	nomatch -> first_match(RE, tl(S), St+1)
    end;
first_match(_, [], _) -> nomatch.
...
split_apply([], _, _, true, []) -> [];
split_apply([], _, _, _, Sub) -> [reverse(Sub)];
...
groups(S, RegExp) when is_list(RegExp) ->
