...
match(Pattern, Str) ->
    Port = port(),
    Bin = term_to_binary({Pattern, Str}),
...
regex_str(Pattern, Str) ->
    Port = port(),
    Bin = term_to_binary({Pattern, Str}),
    Result = port_control(Port, 2, Bin),
    binary_to_term(Result).
    
prematch(Pattern, Str) ->
    Port = port(),
    Bin = term_to_binary({Pattern, Str}),
