...
%%% Original filename: yaws_soap_lib.erl
%%%-------------------------------------------------------------------

%%% modified (WdJ, May 2007): deal with imports in the WSDL.
%%% modified (WdJ, June 2007): renamed the file to erlsoap_lib
...
-include("soap.hrl").
-include("erlsom.hrl").  % FIXME a better solution ?

-record(erlsoap_config, {atts, xsd_path,  user_module, wsdl_file, add_files}).
-record(xsd_file, {atts, name, prefix, import_specs}).
-record(import_specs, {atts, namespace, prefix, location}).

-define(DefaultPrefix, "cwmp").
...
%%% -------------------------------------------------------------------- 
%%% Prepare the WSDL-model
%%% -------------------------------------------------------------------- 
prepareWsdlModel(Envelope) ->
    PrivDir = priv_dir(), 
    prepareWsdlModel(PrivDir,Envelope).

prepareWsdlModel(Path,Envelope) ->
    ...
    {ok, EnvelopeModel} = erlsom:compile_xsd_file(filename:join([Path, Envelope]),
						  [{prefix, "soap"}]),
    {WsdlModel2, EnvelopeModel}.

%%% -------------------------------------------------------------------- 
%%% Parse a WSDL file and return a 'Model'
%%% -------------------------------------------------------------------- 
compileWsdl(WsdlFile, WsdlModel, EnvelopeModel) ->
    compileWsdl(WsdlFile, WsdlModel, EnvelopeModel, []).

compileWsdl(WsdlFile, WsdlModel, EnvelopeModel, Options) ->
    {Model, Operations} = yaws_soap_lib:parseWsdls([WsdlFile], ?DefaultPrefix, WsdlModel, Options, {undefined, []}),
    SoapModel = erlsom:add_model(EnvelopeModel, Model),
    #wsdl{operations = Operations, model = SoapModel}.

%%% -------------------------------------------------------------------- 
%%% Parse a WSDL file and return a 'Model'
%%% This version also parses the WSDL-xsd etc.
%%% -------------------------------------------------------------------- 
initModel(WsdlFile) ->
...
initModelFile(ConfigFile) ->
    ...
    #erlsoap_config{xsd_path = XsdPath, 
		    wsdl_file = Wsdl,
...
priv_dir() -> 
    filename:join([filename:dirname(code:which(erlsoap_lib)),"..", "resources"]).
...
parseWsdls([WsdlFile | Tail], Prefix, WsdlModel, Options, {AccModel, AccOperations}) ->
  ...
  Model2 = case Xsd of
             undefined -> 
               AccModel;
             _ -> 
               {ok, Model} = erlsom_compile:compile_parsed_xsd(Xsd, Options2),
               case AccModel of 
                 undefined -> Model;
                 _ -> io:format("Model: ~P~n", [AccModel, 5]), erlsom:add_model(AccModel, Model)
               end
           end,
...
getXsdFromWsdl(Definitions) ->
  case getTopLevelElements(Definitions, 'wsdl:tTypes') of
    [#'wsdl:tTypes'{choice = [Xsd]}] -> Xsd;
    [] -> undefined
  end.

config_file_xsd() ->
"<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\">"
"  <xs:element name=\"erlsoap_config\">"
"     <xs:complexType>"
"       <xs:sequence>"
