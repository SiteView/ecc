...
makeFault(FaultCode, FaultString,ErrDetail) ->
    Details=case ErrDetail of
                 []->
		    [];
                  _->
		    make_error:faultDetail(ErrDetail)
              end,
try 
    "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:cwmp=\"urn:dslforum-org:cwmp-1-0\">"
     "<soap:Body>"
      "<soap:Fault>"
       ...
       "<faultstring>" ++ FaultString ++ "</faultstring>" ++Details++
      "</soap:Fault>"
     "</soap:Body>"
    "</soap:Envelope>"
  catch
