...
process_v1_v2c_msg(Vsn, _NoteStore, Msg, snmpUDPDomain, 
	...
	Pdu when is_record(Pdu, pdu) ->
        ...
        %% otp update(caining)
        NPdu = (catch add_comunity_vsn(Pdu, Community, Vsn)),
	    {ok, Vsn, NPdu, PduMS, MsgData};
	...
	Trap when is_record(Trap, trappdu) ->
	    ...
	    inc_snmp_in(Trap),
	    MsgData = {Community, sec_model(Vsn)},
        NTrap = (catch add_comunity_vsn_to_v1trap(Trap, Community, Vsn)),
	    {ok, Vsn, NTrap, PduMS, MsgData};
...
%% otp update(caining)
add_comunity_vsn(Pdu = #pdu{}, Community, Vsn) ->
    if 
        Pdu#pdu.type =:= 'snmpv2-trap' ->
            if
                erlang:is_list(Pdu#pdu.varbinds) ->
                    Vbs = Pdu#pdu.varbinds,
                    NPdu = Pdu#pdu{varbinds = (Vbs ++ [{community, Community}, {version, Vsn}])},
                    NPdu;
                true ->
                    Pdu
            end;
...
add_comunity_vsn(OPdu, Community, Vsn) ->
    OPdu.
    
%% otp update(caining)
add_comunity_vsn_to_v1trap(Trap = #trappdu{}, Community, Vsn) ->
    if
        erlang:is_list(Trap#trappdu.varbinds) ->
            Vbs = Trap#trappdu.varbinds,
            NTrap = Trap#trappdu{varbinds = (Vbs ++ [{community, Community}, {version, Vsn}])},
            NTrap;
        true ->
            Trap
    end;
add_comunity_vsn_to_v1trap(OTrap, Community, Vsn) ->
    OTrap.
...
check_sec_module_result({ok, X}, _, _, _, _) ->
    X;
check_sec_module_result({error, Reason, Info}, V3Hdr, Data, _, _) 
  when is_list(Info) ->
    %% case 7.2.6 b
    %% otp update(caining)
    ScopedPdu = (catch get_scoped_pdu(Data)),
    RequestId =(catch get_reqId(ScopedPdu)),
    Infos = Info ++ [{requestid,RequestId}],
    %%
    discard({securityError, Reason, Infos});
...
check_sec_module_result({error, Reason, _ErrorInfo}, V3Hdr, Data, _, _) ->
    ?vtrace("security module result:"
	    "~n   Reason:     ~p"
    ...
    ScopedPdu = (catch get_scoped_pdu(Data)),
    RequestId =(catch get_reqId(ScopedPdu)),
    %%EngineId = (catch get_engineId(V3Hdr)),
    %%(catch set_bootsandtimes(V3Hdr)),
    %%
    discard({securityError, Reason, [{requestid,RequestId}, {v3hdr, V3Hdr}]});
...
set_bootsandtimes(V3Hdr = #v3_hdr{}) ->
    #v3_hdr{msgID                 = MsgID, 
	    msgMaxSize            = MMS, 
	    msgFlags              = MsgFlags,
	    msgSecurityModel      = MsgSecModel,
    ...
    io:format("MsgAuthEngineBoots = ~p~nMsgAuthEngineTime = ~p~n",[MsgAuthEngineBoots, MsgAuthEngineTime]),
    snmpm_config:set_engine_boots(MsgAuthEngineBoots),
    snmpm_config:set_engine_time(MsgAuthEngineTime);
set_bootsandtimes(_) ->
    [].
