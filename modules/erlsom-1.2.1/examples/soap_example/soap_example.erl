...
run() ->
  ...
  %% parse xml
  Xml = filename:join([codeDir(), "SiteviewS3.xsd"]),
  Result = case erlsom:scan_file(Xml, ModelIn) of
             {ok, #'sp:Envelope'{'Body' = #'sp:Body'{choice = Content}}, _} ->
               processContent(Content),
			   io:format("result~p~n",[erlsom:scan_file(Xml, ModelIn)]),
			   io:format("result~p~n",[Content]);
             {error, _} ->
...
compileXSDs() ->
  ...
  %%io:format("soapmodel~p~n",[SoapModel]),
  {ok, ModelIn} = erlsom:add_xsd_file(BodyXsd, [{prefix, "sp"}], SoapModel),
  %%io:format("modelin~p~n",[ModelIn]),
  {ok, ModelOut} = erlsom:add_xsd_file(ResultXsd, [{prefix, "sp"}], SoapModel),
  %%io:format("modelout~p~n",[ModelOut]),
  {ModelIn, ModelOut}.
