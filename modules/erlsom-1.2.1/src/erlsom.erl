...
-export([compile/1, compile/2, compile/3,
	 ...
	 write_xsd_hrl_file/2, write_xsd_hrl_file/3, 
         ...
         add_xsd_file/3,
         add_file/3, add_xsd_model/1, add_model/2, getparams/1]).
...
getparams(Model) ->
    newerlsom_writeHrl:getparams(Model).
    
wsdl_namespace(#model{nss = Nss},Prefix)->
    getns(Nss,Prefix).
    
getns([],_)->"";
getns([{ns,NS,Prefix}|_],Prefix)->
    NS;
getns([_|R],Prefix)->
    getns(R,Prefix).

newwrite_hrl(Model) ->
	Hdr = newerlsom_writeHrl:writeHrl(Model),
	L = string:tokens(Hdr,"\n"),
	rebuild(L).
	
rebuild([]) ->[];
rebuild([F|R]) ->
%%-record('p:setPolicyState', {anyAttribs, 'policyId', 'enable'}).
%%{'p:setPolicyState',undefined,113,false}
%%{"'p:Entity_GetChildNodeCount'"," '_Guid'"}
	case F of
		"-record("++Rest ->
			N1 = string:str(Rest,","),
			MN1 = string:strip(string:sub_string(Rest,1,N1-1)),
			%%È¥µôµ¥ÒýºÅ
			MN2 = list_to_atom(string:sub_string(MN1,2,string:len(MN1)-1)),
			MN3 = [MN2],
			Tuple = string:strip(string:sub_string(F,string:str(F,"{")+1,string:str(F,"}")-1)),
			L = string:tokens(Tuple,","),
			[list_to_tuple(MN3++check(L))]++rebuild(R);
		_->
			rebuild(R)
	end.
...
check([F|R]) ->
	case string:strip(F) of
		"anyAttribs" ->
			[undefined]++check(R);
		Other ->
			[string:sub_string(Other,2,string:len(Other)-1)]++check(R)
	end.
