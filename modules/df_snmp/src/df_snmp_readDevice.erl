...
main(SP_list,Timeout,DTD) ->
    ...
    SP_list2    = getSP_Devid(Devid_list2),
    Mac_list    = readMac_list(SP_list2,Timeout),
    IP_list     = readIP_list(SP_list2,Timeout),
    Msk_list    = readMsk_list(SP_list2,Timeout),
    Inf_list    = readInf_list(SP_list2,Timeout),
    IMI_list    = processIPMskInf(SP_list2,IP_list,Msk_list,Inf_list,[]),
    Devid_list3 = get_device_other(Devid_list2,[],Mac_list,IMI_list),
    
    io:format("test for ~p~n",[Devid_list3]),
    Devid_list3.

getSP_Devid(Devid_list) ->
    lists:map(fun(Devid)->Devid#deviceInfo.sp end, Devid_list).
    

get_devid_SP(SP_list) ->
    lists:map(fun(SP) -> #deviceInfo{snmpFlag=1,sp=SP,community_get=
                                        SP#snmpPara.community} end, SP_list).
