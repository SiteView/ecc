...
%% 	io:format("beamjs_bundle load1111 ~p~p~n", [VM, Bundle]),
	...
	io:format("beamjs_bundle 2222 ~p,    ~p~n", [filename:join(["lib_bundles",Bundle,"__globals__"]), Require]),
	Globals = Require:call([filename:join(["lib_bundles",Bundle,"__globals__"])]),
	io:format("beamjs_bundle load ~p , ~p ~n", [Globals, Require]),
	case Globals of 
		#erlv8_object{} ->
